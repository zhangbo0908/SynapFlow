...
describe('PropertiesPanel', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    ;(useMindmapStore as any).mockImplementation((selector) => selector(mockStore))
  })
  ...
  it('renders global settings when no node is selected', () => {
    ...
    ;(useMindmapStore as any).mockImplementation((selector) => selector(emptyStore))
  ...
  it('calls updateNodeStyle when background color changes', () => {
    ...
    expect(mockUpdateNodeStyle).toHaveBeenCalledWith('node-1', expect.objectContaining({
  ...
  it('uses theme defaults for border color when node has no specific style', () => {
    ...
    ;(useMindmapStore as any).mockImplementation((selector) => selector(businessThemeStore))
