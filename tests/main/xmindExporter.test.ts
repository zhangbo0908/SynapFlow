...
describe('XMind Exporter', () => {
  ...
  it('should update existing XMind file structure', async () => {
    ...
    initialZip.file('content.json', JSON.stringify([{
      id: 'existing-sheet',
      title: 'Old Sheet',
      rootTopic: { id: 'old-root', title: 'Old Root' }
    }]))
    initialZip.file('other.txt', 'keep me')
    const initialBuffer = await initialZip.generateAsync({ type: 'nodebuffer' })
      ...
      sheets: [
        {
          ...
          rootId: 'root-1',
          nodes: {
            'root-1': { id: 'root-1', text: 'Root 1', x: 0, y: 0, width: 100, height: 40, children: [], isRoot: true }
          },
          theme: 'default',
        ...
        },
        {
          ...
          rootId: 'root-2',
          nodes: {
            'root-2': { id: 'root-2', text: 'Root 2', x: 0, y: 0, width: 100, height: 40, children: [], isRoot: true }
          },
          theme: 'default',
          editorState: { zoom: 1, offset: { x: 0, y: 0 } }
        }
