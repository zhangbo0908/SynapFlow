...
基于 XMind 的美学标准，针对不同场景（商务、创意、学习）设计的核心主题预设。

### 6.1 商务专业 (Professional Business)
**设计逻辑**：强调**权威、冷静、高效**。采用低饱和度的冷色调，配合硬朗的线条，适合正式汇报与项目管理。
- **配色方案 (Palette)**：
  - **画布**：`#F5F7FA` (浅灰蓝，降低视觉疲劳)
  - **中心主题**：背景 `#2C3E50` (深蓝灰) / 文字 `#FFFFFF`
  - **分支色系**：`#34495E` (灰蓝), `#7F8C8D` (中灰), `#2980B9` (稳重蓝), `#16A085` (青绿)
- **形状模板**：
...
- **配色方案 (Palette)**：
  - **画布**：`#FFFFFF` (纯白)
  - **中心主题**：背景 `#4ECDC4` (薄荷绿) / 文字 `#FFFFFF`
  - **分支色系**：`#FF6B6B` (暖红), `#FFE66D` (明黄), `#1A535C` (深青), `#F7FFF7` (极浅绿)
- **形状模板**：
  - **中心**：胶囊形 (Capsule) / 椭圆
  - **分支**：圆角矩形 (`16px` Radius)
...
- **配色方案 (Palette)**：
  - **画布**：`#FFFFFF`
  - **中心主题**：背景 `#000000` / 文字 `#FFFFFF`
  - **分支色系**：全黑 (`#000000`) 或 深灰 (`#333333`)
- **形状模板**：
  - **中心**：圆角矩形 (`8px`)
  - **分支**：仅下划线 (Underline)
  - **连线**：细直线 (`1px`)
...
- **配色方案 (Palette)**：
  - **画布**：`#FFF5F0` (暖白)
  - **中心主题**：背景 `#FF9F43` (活力橙) / 文字 `#FFFFFF`
  - **分支色系**：`#EE5253` (朱红), `#FECA57` (日光黄), `#FF9FF3` (粉紫), `#54A0FF` (亮蓝)
- **形状模板**：
  - **中心**：云朵形或不规则圆形
  - **分支**：圆角矩形
  - **连线**：粗曲线，富有张力
...
### 6.5 暗夜霓虹 (Dark Neon)
**设计逻辑**：强调**沉浸、科技感**。深色模式专用，高饱和度荧光色，适合夜间编程思考或系统架构设计。
- **配色方案 (Palette)**：
  - **画布**：`#1E1E1E` (深灰黑)
  - **中心主题**：边框霓虹光晕 / 背景透明 / 文字 `#FFFFFF`
  - **分支色系**：`#00FF9D` (荧光绿), `#FF0055` (荧光红), `#00D2FF` (荧光蓝), `#F0F` (荧光紫)
- **形状模板**：
  - **中心**：六边形 (Hexagon) 或 菱形
