...
2. **布局平滑过渡 (Layout Transition)**：
   - 触发场景：节点增删、拖拽重组、展开/折叠。
   - 动效参数：`0.3s cubic-bezier(0.25, 0.1, 0.25, 1.0)`。
   - 效果：所有受影响节点的坐标变化应呈现连续的物理移动轨迹，避免视觉跳变。
3. **拖拽重组 (Drag & Drop)**：
   - **替身 (Ghost)**：拖拽时显示当前节点的半透明快照 (Opacity 50%)。
   - **吸附反馈 (Drop Target)**：当替身进入有效吸附区域时，目标节点显示 `2px` `ui-accent-primary` 高亮边框。

---

## 5. 辅助功能与无障碍 (Accessibility)
- **对比度**：确保所有分支颜色在白色背景上的对比度符合 WCAG AA 标准。
- **全键盘导航 (Full Keyboard Navigation)**：
  - `Arrow Right`：跳转到当前节点的第一个子节点（可视中心最近）。
  - `Arrow Left`：跳转到父节点。
  - `Arrow Up`：跳转到上一个同级兄弟节点。
  - `Arrow Down`：跳转到下一个同级兄弟节点。
