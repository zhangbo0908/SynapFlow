...
const NodeComponent: React.FC<NodeComponentProps> = React.memo(
  ({ nodeId, node: propNode, styleConfig, onNodeDragStart, isDropTarget }) => {
    ...
    const commonProps = {
      ...
      style: node.style?.shadowColor
        ? {
            filter: `drop-shadow(0 0 ${node.style.shadowBlur || 5}px ${node.style.shadowColor})`,
          }
        : undefined,
    };
        ...
        {isEditing ? (
          <foreignObject x={0} y={0} width={node.width} height={node.height}>
            <input
              ...
              style={{
                color: node.style?.color || "var(--color-text-node)",
