...
import { saveMindmap, openMindmap, saveMarkdown, saveImage, savePdf } from './fileSystem'
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('user:updatePreferences', (_, prefs: Partial<UserPreferences>) => {
  ...
  ipcMain.handle('file:save', async (_, data: LocalMindmap, filePath?: string) => {
  ...
  ipcMain.handle('file:saveImage', async (_, dataUrl: string, format: 'png' | 'jpeg') => {
  ...
  ipcMain.handle('file:importXMind', async () => {
    ...
    try {
      ...
      if (error instanceof Error && error.message === 'XMIND_XML_NOT_SUPPORTED') {
        dialog.showMessageBox({
          type: 'error',
          title: '格式不支持 / Format Not Supported',
          message: '检测到旧版 XMind (XML) 格式。',
          detail: 'SynapFlow 仅支持新版 XMind (JSON) 格式。请使用 XMind Zen 或 XMind 2020+ 另存文件后再试。'
        })
      }
